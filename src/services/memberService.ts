...
const updatePayload = (input: MemberUpdateInput) => ({
...
const createPayload = (input: MemberInput) => ({
  ...
  contribution: {
    ...
    ...(input.contribution.frequency === 'CUSTOM' && {
      interval_value: input.contribution.intervalValue,
    }),
    ...
    ...(input.contribution.lateFeeEnabled && {
      late_fee_percentage: input.contribution.lateFeePercentage,
    }),
...
export async function getMember(id: string) {
  const { data } = await apiClient.get<MemberDto>(`/admin/users/${id}`)
  return mapMember(data)
}
export async function createMember(input: MemberInput) {
  const { data } = await apiClient.post<MemberDto>('/admin/users/', createPayload(input))
  return mapMember(data)
}
export async function updateMember(id: string, input: MemberUpdateInput) {
  const { data } = await apiClient.patch<ApiEnvelope<MemberDto>>(
    `/admin/users/${id}`,
    updatePayload(input),
  )
...
export async function updateMemberStatus(id: string, status: MemberStatus) {
  if (status === 'INACTIVE')
    throw new Error('INACTIVE status cannot be set through the backend status endpoints')
  const action = status === 'SUSPENDED' ? 'suspend' : 'activate'
  const { data } = await apiClient.post<ApiEnvelope<{ id: string; status: MemberStatus }>>(
