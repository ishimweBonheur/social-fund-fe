...
export async function listPendingContributions() { const { data } = await apiClient.get<{ data: ContributionDto[] }>('/admin/contributions/pending', { params: { limit: 100 } }); return data.data.map(map) }
