    ...
    <div>
      ...
      <Card>
        <CardContent className="divide-y p-0">
          {notifications.length === 0 ? <EmptyState title="No notifications" description="You are all caught up. New fund activity will appear here." /> : notifications.map((notification) => (
            <button key={notification.id} type="button" onClick={() => { setSelected(notification); markNotificationRead(notification.id) }} className="block w-full p-4 text-left hover:bg-muted">
              <div className="flex justify-between gap-4">
                <p className="text-sm font-semibold">{notification.title}</p>
                <span className="text-xs text-muted-foreground">{notification.time}</span>
              </div>
              <p className="mt-1 text-sm text-muted-foreground">{notification.message}</p>{notification.deliveryStatus === 'FAILED' && <Button className="mt-2" size="sm" variant="outline" onClick={(event) => { event.stopPropagation(); void retryNotification(notification.id).then(() => toast.success('Notification queued for retry.')).catch(() => toast.error('Unable to retry notification.')) }}>Retry Delivery</Button>}
            </button>
          ))}
        </CardContent>
      </Card>
